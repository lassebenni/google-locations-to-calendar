...
class WeatherEvent:
    ...
    def __post_init__(self):
        ...
        if self.forecast:
            self.summary = (
                f"Weeronline: {self.forecast.digit} - {self.forecast.temperature}°C"
            )
...
class WeatherToGoogleCalendar(object):
    ...
    def create_weather_forecasts(self):
        ...
        for forecast in forecasts:
            ...
            if existing_forecast:
                updated_forecast = WeatherEvent(
                    forecast=forecast, calendar_event=existing_forecast.calendar_event
                )

                if existing_forecast.calendar_event.summary != updated_forecast.summary:
                    updated_forecast.calendar_event.body[
                        "summary"
                    ] = updated_forecast.summary
    ...
    def list_weather_events(self, start_date: str = "") -> List[WeatherEvent]:
        ...
        events = self.calendar.list_events(
            query="Weeronline", min_datetime=tz_iso_stamp
        )
    ...
    def add_weather_event(self, event: WeatherEvent):
        calendar_event = self.calendar.create_calendar_event(
